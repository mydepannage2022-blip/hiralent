...
const blogPosts = [
  {
    id: 1, scene: "matching" as const,
    title: "How AI Matching Actually Works at Hiralent",
    author: "Hiralent Team", date: "Feb 2026", readTime: "5 min",
    tags: ["AI", "Matching"], accent: "#005DDC",
    description: "We don't match on keywords. Here's how Hiralent scores candidates on real skills, experience depth, and role fit — with full transparency.",
  },
  {
    id: 2, scene: "profile" as const,
    title: "Build a Profile That Employers Actually Trust",
    author: "Hiralent Team", date: "Jan 2026", readTime: "4 min",
    tags: ["Profile", "Tips"], accent: "#00A35A",
    description: "Your CV is a PDF. Your Hiralent profile is a verified, structured signal. Learn how to go from uploaded resume to 90+ profile score.",
  },
  {
    id: 3, scene: "assessment" as const,
    title: "What to Expect in a Hiralent Assessment",
    author: "Hiralent Team", date: "Jan 2026", readTime: "6 min",
    tags: ["Assessment", "Guide"], accent: "#7C3AED",
    description: "Real code. Real time. Real feedback. Here's how our skill assessments work, what we measure, and how to prepare.",
  },
];
    ...
    <div className="relative w-full flex justify-center items-center bg-white py-16 md:py-20 overflow-hidden">
      ...
      <div className="relative mx-auto w-[92%] max-w-7xl flex flex-col justify-center items-center gap-10 md:gap-12">
        ...
        <motion.div
          ...
          variants={headingVariants} initial="hidden"
        ...
        >
          ...
          >
            No fluff. Practical guides on profiles, matching, and assessments — written by the team building the platform.
          </motion.p>
        </motion.div>
            ...
            >
              ...
              <div className="p-5 md:p-6 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  {post.tags.map((tag) => (
                    <span key={tag} className="px-2 py-0.5 rounded-md text-[10px] font-bold border"
                      style={{ color: post.accent, borderColor: `${post.accent}22`, backgroundColor: `${post.accent}08` }}>
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <p className="text-[#64748B] text-[13px] leading-relaxed line-clamp-3">{post.description}</p>

                {/* ✅ Read More → goes to login */}
        ...
        {/* ✅ View All Posts → goes to login */}
        <motion.div
          className="text-center mt-2"
          initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.45 }}
        >
          <Link href="/auth/login">
            <motion.div
              className="inline-flex items-center gap-2 rounded-2xl bg-[#005DDC] text-white font-semibold py-3.5 px-6 shadow-[0_18px_40px_-28px_rgba(0,93,220,0.45)] hover:opacity-95 transition cursor-pointer"
          ...
          <div className="text-[11px] text-[#94A3B8] mt-2 font-semibold">Fresh updates weekly</div>
        </motion.div>
      </div>
    </div>
  );
...
function SceneMatching() {
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[#F8FBFF] via-[#EEF4FF] to-[#F0F4FF]">
      <div className="absolute inset-0 opacity-[0.035]" style={{ backgroundImage: "radial-gradient(circle, #005DDC 0.8px, transparent 0.8px)", backgroundSize: "24px 24px" }} />
      {[80,56,34].map((size, i) => (
        <motion.div key={i} className="absolute rounded-full border border-[#005DDC]"
          style={{ width: size, height: size, right: 108 - size/2, top: "50%", marginTop: -size/2 }}
          animate={{ opacity: [0.06,0.18,0.06], scale: [1,1.04,1] }}
          transition={{ duration: 2.5, delay: i*0.4, repeat: Infinity, ease: "easeInOut" }} />
      ))}
      {skills.map((s, i) => (
        <motion.div key={s.label} className="absolute flex items-center gap-2" style={{ left: 14, top: s.y }}
          initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }} transition={{ delay: 0.25 + i*0.09, duration: 0.35 }}>
          <span className="h-2.5 w-2.5 rounded-full bg-[#005DDC] ring-[3px] ring-[#005DDC]/10 flex-shrink-0" />
          <span className="rounded-lg bg-white border border-[#E6ECF8] px-2 py-[3px] text-[10px] font-bold text-[#0b1b3a] shadow-sm whitespace-nowrap">{s.label}</span>
          <div className="w-8 h-[3px] rounded-full bg-[#E6ECF8] overflow-hidden">
            <motion.div className="h-full rounded-full bg-[#005DDC]"
              initial={{ width: 0 }} whileInView={{ width: `${s.score}%` }}
              viewport={{ once: true }} transition={{ delay: 0.6 + i*0.08, duration: 0.5 }} />
          </div>
        </motion.div>
      ))}
      <svg className="absolute inset-0 w-full h-full" style={{ zIndex: 1 }}>
        {skills.map((s, i) => (
          <motion.line key={i} x1={125} y1={s.y+10} x2={225} y2={105} stroke="#005DDC" strokeWidth="1.2" strokeDasharray="3 4"
            initial={{ strokeOpacity: 0 }} whileInView={{ strokeOpacity: 0.2 }}
            viewport={{ once: true }} transition={{ delay: 0.5 + i*0.08, duration: 0.4 }} />
        ))}
      </svg>
      <motion.div className="absolute right-4 top-1/2 -translate-y-1/2 w-[130px] rounded-2xl border border-[#E6ECF8] bg-white p-3 shadow-[0_8px_28px_-10px_rgba(0,0,0,0.1)]"
        style={{ zIndex: 2 }} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }}
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            <div className="text-[10px] font-bold text-[#0b1b3a] leading-tight">Full-Stack Dev</div>
        ...
        <div className="rounded-xl bg-[#005DDC]/[0.05] border border-[#005DDC]/10 p-2.5 flex items-center justify-center gap-3">
          <div className="relative w-10 h-10">
            <svg viewBox="0 0 36 36" className="w-10 h-10 -rotate-90">
              ...
              <motion.circle cx="18" cy="18" r="15" fill="none" stroke="#005DDC" strokeWidth="3"
                strokeLinecap="round" strokeDasharray="94.25" strokeDashoffset="94.25"
                whileInView={{ strokeDashoffset: 94.25*(1-0.94) }} viewport={{ once: true }}
                transition={{ delay: 0.8, duration: 0.9, ease: "easeOut" }} />
            </svg>
            <span className="absolute inset-0 flex items-center justify-center text-[11px] font-extrabold text-[#005DDC]">94</span>
          </div>
          <div>
        ...
        <div className="mt-2 flex items-center gap-1">
          ...
          <span className="text-[8px] font-semibold text-[#64748B]">Skills verified</span>
        </div>
      </motion.div>
    </div>
  );
...
function SceneProfile() {
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[#F6FFF6] via-[#F0FBF0] to-[#F7FBFF]">
      <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: "radial-gradient(#00A35A 0.8px, transparent 0.8px)", backgroundSize: "20px 20px" }} />
      <motion.div className="absolute left-4 top-1/2 -translate-y-1/2 w-[85px]"
        ...
        <div className="rounded-xl border border-[#E6ECF8] bg-white p-2.5 shadow-sm">
          ...
          {[100,80,95,60,85,70].map((w, i) => (
            <motion.div key={i} className="h-[2.5px] rounded-full bg-[#E2E8F0] mb-[4px]" style={{ width: `${w}%` }}
        ...
        <AnimatePresence>
          {phase === 1 && (
            <motion.div className="mt-2 rounded-full bg-[#00A35A] px-2 py-0.5 text-[8px] font-bold text-white text-center flex items-center justify-center gap-1"
              initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              <Cpu className="h-2.5 w-2.5 animate-spin" />Parsing…
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
      <motion.div className="absolute left-[102px] top-1/2 -translate-y-1/2 flex items-center"
        initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.3 }}>
        {[0,1,2].map((i) => (
          <motion.span key={i} className="h-1 w-1 rounded-full bg-[#00A35A] mx-[2px]"
            animate={{ opacity: [0.2,0.8,0.2], x: [0,6,0] }}
            transition={{ duration: 1.2, delay: i*0.2, repeat: Infinity, ease: "easeInOut" }} />
        ))}
        <ArrowRight className="h-3 w-3 text-[#00A35A]/40 ml-1" />
      </motion.div>
      <motion.div className="absolute right-3 top-1/2 -translate-y-1/2 w-[165px] rounded-2xl border border-[#E6ECF8] bg-white p-3.5 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.1)]"
        initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }} transition={{ delay: 0.5, duration: 0.4 }}>
        ...
        <div className="rounded-lg bg-[#F7FBF7] border border-[#E6F0E6] p-2 mb-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[9px] font-semibold text-[#64748B]">Profile strength</span>
            <motion.span className="text-[11px] font-extrabold text-[#00A35A]"
          ...
          <div className="h-1.5 w-full rounded-full bg-[#E6ECF8] overflow-hidden">
            <motion.div className="h-full rounded-full bg-[#00A35A]"
              initial={{ width: "0%" }} animate={phase >= 2 ? { width: "92%" } : { width: "0%" }}
              transition={{ duration: 0.8, ease: "easeOut" }} />
          </div>
        </div>
        <div className="flex flex-wrap gap-1">
          {extractedSkills.map((s, i) => (
            <motion.span key={s} className="px-1.5 py-[2px] rounded text-[8px] font-bold bg-[#F7FBFF] border border-[#E6ECF8] text-[#0b1b3a]"
              initial={{ opacity: 0, scale: 0.7 }} animate={phase >= 1 ? { opacity: 1, scale: 1 } : {}}
        ...
        <AnimatePresence>
          {phase >= 2 && (
            <motion.div className="absolute -top-2.5 -right-2.5 h-7 w-7 rounded-full bg-[#00A35A] flex items-center justify-center shadow-[0_4px_14px_rgba(0,163,90,0.35)]"
      ...
      <AnimatePresence>
        {phase >= 2 && (
          <motion.div className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-[#0b1b3a]/80 backdrop-blur px-3 py-1 text-[10px] font-bold text-white flex items-center gap-1.5"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.3 }}>
            <Zap className="h-3 w-3 text-[#00A35A]" />24 skills extracted
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function SceneAssessment() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setLineIndex((p) => p+1), 200 + Math.random()*120);
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[#FAF5FF] via-[#F5F0FF] to-[#F7FBFF]">
      <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: "linear-gradient(#7C3AED 1px, transparent 1px), linear-gradient(90deg, #7C3AED 1px, transparent 1px)", backgroundSize: "28px 28px" }} />
      <motion.div className="absolute left-3 right-3 top-3 bottom-10 rounded-xl border border-[#E6ECF8] bg-[#0C0F1A] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col"
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-white/[0.06]">
          ...
          <div className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded bg-white/[0.05] border border-white/[0.06]">
            ...
            <span className="text-[9px] font-mono text-white/40">{done ? "02:18" : "live"}</span>
          </div>
        </div>
        <div className="flex-1 px-3 py-2 font-mono text-[10px] leading-[1.8] overflow-hidden">
          {lines.slice(0, lineIndex).map((line, i) => (
            <motion.div key={i} className="flex items-center gap-2"
              ...
              <span className="text-white/10 w-3 text-right select-none text-[9px]">{i+1}</span>
              <span style={{ color: line.color }}>{line.text}</span>
            </motion.div>
          ))}
          {!done && (
            <motion.span className="inline-block w-[5px] h-3 bg-[#7C3AED] ml-5 rounded-[1px]"
      ...
      <AnimatePresence>
        {done && (
          <motion.div className="absolute bottom-2.5 left-3 right-3 flex items-center justify-between"
            initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.3 }}>
            <div className="rounded-xl border border-[#E6ECF8] bg-white px-3 py-1.5 shadow-sm flex items-center gap-2.5">
              <div className="flex gap-[3px]">
                {[1,2,3,4,5].map((n) => (
                  <motion.div key={n} className={`h-2 w-2 rounded-full ${n<=4?"bg-[#00A35A]":"bg-[#E6ECF8]"}`}
                    initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2+n*0.06 }} />
                ))}
              </div>
              <span className="text-[10px] font-bold text-[#0b1b3a]">4/5 passed</span>
              <span className="text-[9px] text-[#94A3B8]">· 138ms</span>
            </div>
            <motion.div className="rounded-full px-2.5 py-1 flex items-center gap-1.5 shadow-[0_4px_12px_rgba(124,58,237,0.2)]"
