    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-2">
          {isRecommended && (
            <span className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-[#005DDC] text-[8px] rounded-md">
              <Star className="w-3 h-3" />
              Recommended
            </span>
          )}
          {isCompleted && (
            <span className="flex items-center gap-1 px-2 py-1 bg-green-50 text-green-600 text-[8px] rounded-md">
              <CheckCircle className="w-3 h-3" />
              Completed
            </span>
          )}
        </div>
        <span className={`px-2 py-1 text-[8px] rounded-md ${difficultyConfig.color}`}>
      ...
      {/* Description */}
      <p className="text-[#757575] text-[10px] mb-4 leading-relaxed">
        {description}
      </p>
      ...
      <div className="flex items-center gap-4 mb-4 text-sm text-[#757575]">
        <div className="flex items-center gap-1">
          ...
          <span className='text-[10px]'>{questionCount} questions</span>
        </div>
        <div className="flex items-center gap-1">
          <Clock className="w-3 h-3" />
          <span className='text-[10px]'>{timeEstimate}</span>
        </div>
      </div>
      ...
      {isCompleted && lastScore !== undefined && (
        <div className="flex items-center gap-2 mb-4 p-3 bg-gray-50 rounded-md">
          <span className="text-sm text-[#757575]">Last Score:</span>
          <span className="font-semibold text-[#005DDC]">{lastScore}%</span>
        </div>
      )}
