...
// Allowed tags (keeps compatibility if any file still references Apply Result)
...
const tagStyles: Record<NotificationTag, string> = {
    ...
    Meeting: 'text-[#009E00] border border-[#009E00]',
    'New Applications': 'text-[#005DDC] border border-[#005DDC]',
    'Apply Result': 'text-[#8A2BE2] border border-[#8A2BE2]',
};
...
export default function NotificationItem({
    ...
    return (
        <div
            className={`group relative flex items-start gap-3 sm:gap-4 border border-[#EDEDED] px-3 py-4 sm:p-4 transition
        ...
        >
            ...
            {/* Main content */}
            <div className="flex-1 pr-10 sm:pr-28">
            ...
            {/* Top-right icons (star + mail) */}
            <div className="absolute right-4 top-3 flex items-center gap-1 sm:gap-3">
                {/* Star: visible only on hover when inactive; visible always when starred */}
                <button
                ...
                {/* Mail icon: visible always when unread; if read -> visible on hover */}
                <button
