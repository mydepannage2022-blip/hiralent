...
export async function createJob(req: Request, res: Response) {
  try {
    ...
    if (!isCompanyUser(user)) {
      return res
        .status(403)
        .json({ success: false, message: 'Forbidden: company role required' });
    }
    ...
    if (!companyId) {
      return res
        .status(400)
        .json({ success: false, message: 'Missing company_id in auth token' });
    }

    const data: CreateJobRequest = req.body;
    const job = await createJobService(companyId, data);
...
export async function listJobs(req: Request, res: Response) {
  try {
    ...
    if (isCompanyUser(user)) {
      ...
      if (isSuperadmin(user) && q.company_id) {
        filters.company_id = String(q.company_id);
      }
    } else {
      // Public → uniquement jobs actifs
    ...
    const result = await listJobsService(filters);
...
export async function getJobById(
  ...
  try {
    ...
    if (!isSuperadmin(user)) {
      if (isCompanyUser(user)) {
        // Company user ne peut voir que ses jobs
        ...
        // Public ne peut voir que les jobs actifs
...
export async function patchJobStatus(
  ...
  try {
    ...
    const updated = await patchJobStatusService(req.params.id, status);
    return res.json({ success: true, data: updated });
  } catch (err: any) {
    return res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (!isCompanyUser(user)) {
      return res
        ...
        .json({
          success: false,
          message: 'Forbidden: company role required',
        });
    }
    ...
    if (!companyId) {
      return res
        ...
        .json({
          success: false,
          message: 'Missing company_id in auth token',
        });
    }
