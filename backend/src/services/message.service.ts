...
export class MessageService {
   ...
   */
  async getConversations(userId: string, archived: boolean = false): Promise<ConversationResponse[]> {
    try {
      const conversations = await prisma.conversation.findMany({
        where: {
          ...
          AND: [
            userId === conversations?.participant_1_id 
   ...
   */
  async createConversation(userId: string, input: CreateConversationInput): Promise<{ conversation_id: string }> {
    try {
      ...
      if (initial_message) {
        await this.sendMessage(userId, conversation.conversation_id, {
   ...
   */
  async sendMessage(userId: string, conversationId: string, input: SendMessageInput): Promise<MessageResponse> {
    try {
   ...
   */
  async getMessages(userId: string, conversationId: string, query: GetMessagesQuery): Promise<MessageResponse[]> {
    try {
      // Verify conversation exists and user is participant
      const conversation = await prisma.conversation.findFirst({
        where: {
   ...
   */
  async markMessagesAsRead(userId: string, input: MarkReadInput): Promise<{ updated_count: number }> {
    try {
   ...
   */
  async archiveConversation(userId: string, conversationId: string, input: ArchiveConversationInput): Promise<{ success: boolean }> {
    try {
      // Verify conversation exists and user is participant
      const conversation = await prisma.conversation.findFirst({
        where: {
