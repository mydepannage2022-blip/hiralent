...
export class CandidateService {
  ...
  async uploadAndProcessCV(candidateId: string, file: Express.Multer.File) {
    try {
      ...
      this.processDocumentAsync(document.document_id, candidateId);
  ...
  private async processDocumentAsync(documentId: string, candidateId: string) {
    try {
      ...
      await this.updateCandidateProfile(candidateId, extractedData);

      // Generate career prediction
      await this.generateCareerPrediction(candidateId);

      // Create/update candidate vector for job matching
      await this.updateCandidateVector(candidateId);
  ...
  // Update candidate profile with extracted data
  private async updateCandidateProfile(candidateId: string, extractedData: any) {
    try {
  ...
  async generateCareerPrediction(candidateId: string) {
    try {
      // Get candidate data
  ...
  // Create/update candidate vector for job matching
  async updateCandidateVector(candidateId: string) {
    try {
      const candidate = await prisma.user.findUnique({
        where: { user_id: candidateId },
        include: {
  ...
  async getJobRecommendations(candidateId: string, limit: number = 20) {
    try {
      ...
      if (!candidateVector) {
        ...
        await this.updateCandidateVector(candidateId);
        return this.getJobRecommendations(candidateId, limit);
      }
  ...
  // Calculate profile completeness
  async calculateProfileCompleteness(candidateId: string) {
    try {
      const candidate = await prisma.user.findUnique({
        where: { user_id: candidateId },
        include: {
  ...
  async getProfileSummary(candidateId: string) {
    try {
      const candidate = await prisma.user.findUnique({
        where: { user_id: candidateId },
        include: {
