...
exports.validateTimeLimit = exports.validateQuestionSubmission = exports.checkAssessmentStatus = exports.validateAssessmentOwnership = void 0;
const prisma_1 = __importDefault(require("../lib/prisma"));
// ==================== ASSESSMENT OWNERSHIP VALIDATION ====================
const validateAssessmentOwnership = async (req, res, next) => {
    try {
        ...
        const assessment = await prisma_1.default.skillAssessment.findUnique({
...
exports.validateAssessmentOwnership = validateAssessmentOwnership;
// ==================== ASSESSMENT STATUS VALIDATION ====================
const checkAssessmentStatus = async (req, res, next) => {
    try {
        const assessmentId = req.params.assessmentId;
...
exports.checkAssessmentStatus = checkAssessmentStatus;
// ==================== QUESTION SUBMISSION VALIDATION ====================
const validateQuestionSubmission = async (req, res, next) => {
    try {
        const assessmentId = req.params.assessmentId;
...
exports.validateQuestionSubmission = validateQuestionSubmission;
// ==================== TIME LIMIT VALIDATION ====================
const validateTimeLimit = async (req, res, next) => {
    try {
...
exports.validateTimeLimit = validateTimeLimit;
// ==================== HELPER FUNCTIONS ====================
function getOperationFromRoute(routePath) {
    if (routePath.includes('/question'))
        return 'GET_QUESTION';
    if (routePath.includes('/answer'))
        return 'SUBMIT_ANSWER';
    if (routePath.includes('/complete'))
        return 'COMPLETE_ASSESSMENT';
    if (routePath.includes('/progress'))
        return 'GET_PROGRESS';
    if (routePath.includes('/results'))
        return 'GET_RESULTS';
    return 'UNKNOWN';
}
function validateStatusForOperation(status, operation) {
    const rules = {
    ...
    const allowedStatuses = rules[operation] || [];
    ...
    return {
        allowed: false,
        message: messages[status] || `Operation not allowed for status: ${status}`
    };
}
