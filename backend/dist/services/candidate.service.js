...
exports.getCandidateProfile = exports.uploadProfilePicture = exports.updateCandidateHeadline = exports.getProfileSummary = exports.calculateProfileCompleteness = exports.getJobRecommendations = exports.hasExistingResume = exports.deleteResume = exports.getResumeDownloadUrl = exports.uploadAndProcessCV = void 0;
exports.updateCandidateLocation = updateCandidateLocation;
exports.updateCandidateSalary = updateCandidateSalary;
...
const documentProcessor_service_1 = require("./candidate/documentProcessor.service");
const cleanup_service_1 = require("./candidate/cleanup.service");
const prisma = new client_1.PrismaClient();
cloudinary_1.v2.config({
    cloudinary_url: process.env.CLOUDINARY_URL,
});
const uploadAndProcessCV = async (candidateId, file) => {
    try {
        ...
        await (0, cleanup_service_1.cleanupOldResume)(candidateId);
        ...
        (0, documentProcessor_service_1.processDocumentAsync)(document.document_id, candidateId, cloudinaryResult.secure_url);
...
exports.uploadAndProcessCV = uploadAndProcessCV;
const getResumeDownloadUrl = async (candidateId) => {
    try {
...
const deleteResume = async (candidateId) => {
    try {
        ...
        await (0, exports.calculateProfileCompleteness)(candidateId);
        return {
            success: true,
            data: {},
...
const hasExistingResume = async (candidateId) => {
    try {
...
exports.hasExistingResume = hasExistingResume;
const getJobRecommendations = async (candidateId, limit = 20) => {
    try {
        ...
        const combinedVector = candidateVector.combined_vector;
        // Find similar jobs using Pinecone
        const similarJobs = await (0, pinecone_1.findSimilarJobs)(combinedVector, limit);
        ...
        for (const match of similarJobs) {
            ...
            const matchReasoning = await (0, openai_1.generateJobMatchReasoning)(candidateSkills, jobRequirements);
            // Save recommendation
        ...
        const serviceError = new Error("Failed to get job recommendations");
...
const calculateProfileCompleteness = async (candidateId) => {
    try {
        ...
        if (candidate.email?.trim())
            basicInfoScore += 5;
        if (candidate.phone_number?.trim())
            basicInfoScore += 4;
        else
        ...
        let documentScore = 0;
        const hasResume = candidate.candidateDocuments.some(doc => doc.upload_status === 'completed' && doc.extraction_status === 'completed');
        if (hasResume) {
            documentScore = 5;
        }
        else {
...
const getProfileSummary = async (candidateId) => {
    try {
        const candidate = await prisma.user.findUnique({
            where: { user_id: candidateId },
            include: {
        ...
        if (candidate.profileCompleteness) {
            profileCompleteness = {
                ...
                missing_fields: Array.isArray(candidate.profileCompleteness.missing_fields)
                    ? candidate.profileCompleteness.missing_fields
                    : JSON.parse(candidate.profileCompleteness.missing_fields),
                suggestions: Array.isArray(candidate.profileCompleteness.suggestions)
                    ? candidate.profileCompleteness.suggestions
                    : JSON.parse(candidate.profileCompleteness.suggestions)
            };
        }
        // Build summary
        const summary = {
            ...
            career_prediction: candidate.careerPredictions[0] ? {
                ...
                predicted_roles: JSON.parse(candidate.careerPredictions[0].predicted_roles),
                career_path: JSON.parse(candidate.careerPredictions[0].career_path),
                skill_gaps: JSON.parse(candidate.careerPredictions[0].skill_gaps),
                salary_prediction: JSON.parse(candidate.careerPredictions[0].salary_prediction),
...
const updateCandidateHeadline = async (candidateId, data) => {
    try {
        ...
        (0, exports.calculateProfileCompleteness)(candidateId).catch((error) => {
        ...
        throw new Error(`Failed to update headline: ${error.message || "Unknown error"}`);
    }
};
exports.updateCandidateHeadline = updateCandidateHeadline;
async function updateCandidateLocation(userId, data) {
    try {
...
async function updateCandidateSalary(userId, data) {
    try {
        const result = await prisma.candidateProfile.upsert({
            where: { candidate_id: userId },
            update: {
...
const uploadProfilePicture = async (candidateId, file) => {
    ...
    try {
        ...
        console.log(`Processing profile picture upload for candidate: ${candidateId}`);
        ...
        const cloudinaryResult = await cloudinary_1.v2.uploader.upload(tempFilePath, {
        ...
        (0, cleanup_service_1.cleanupTempFile)(tempFilePath);
        // Delete old Cloudinary image if exists (using separated service)
        if (oldPictureUrl && oldPictureUrl !== cloudinaryResult.secure_url) {
            await (0, cleanup_service_1.cleanupOldProfilePicture)(candidateId, oldPictureUrl);
        }
        // Recalculate profile completeness (async, don't wait)
        (0, exports.calculateProfileCompleteness)(candidateId).catch((error) => {
            console.warn("Failed to recalculate profile completeness:", error);
...
const getCandidateProfile = async (candidateId) => {
    try {
        ...
        const serviceError = new Error("Failed to get candidate profile");
