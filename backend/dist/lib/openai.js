...
exports.generateGeminiJSON = generateGeminiJSON;
exports.extractSkillsFromText = extractSkillsFromText;
exports.predictCareerPath = predictCareerPath;
exports.generateJobMatchReasoning = generateJobMatchReasoning;
exports.createEmbedding = createEmbedding;
exports.generateSkillsAssessmentJSON = generateSkillsAssessmentJSON;
...
async function generateGeminiJSON(systemPrompt, userPrompt, retries = 3) {
    let lastError;
...
// ---------------------- predictCareerPath ----------------------
async function predictCareerPath(candidateData) {
    try {
...
async function generateJobMatchReasoning(candidateSkills, jobRequirements) {
    try {
...
async function generateSkillsAssessmentJSON(systemPrompt, userPrompt, assessmentType = 'questions') {
    try {
...
function isValidResponse(data, type) {
    switch (type) {
        case 'questions':
...
async function getSmartFallbackResponse(type, originalPrompt) {
    try {
...
function simplifyOriginalPrompt(originalPrompt, type) {
...
function getContextualStaticFallback(type, originalPrompt) {
...
function generateContextualQuestions(context) {
...
function generateContextualEvaluation(context) {
    const { skill } = context;
    return {
        score: 75,
...
function generateContextualDifficulty(context) {
...
function generateContextualReport(context) {
...
function getNextDifficultyLevel(current, experience) {
    const progression = {
    ...
    if (experience.toLowerCase().includes('senior') || experience.toLowerCase().includes('expert')) {
        return progression[current] || 'ADVANCED';
    }
