...
# ⭐⭐⭐ IMPORTS GEMINI AVEC GESTION D'ERREURS AMÉLIORÉE ⭐⭐⭐
...
# ⭐⭐⭐ IMPORTS WEB SCRAPING AVEC GESTION D'ERREURS AMÉLIORÉE ⭐⭐⭐
...
try:
    ...
    # Essayer d'importer depuis le dossier crawler
    crawler_path = os.path.join(current_dir, "crawler")
    ...
    if hasattr(web_scraping_service, 'get_scraping_status'):
        SCRAPING_AVAILABLE = True
        print("✅ Web scraping module loaded successfully")
    else:
        print("⚠️ Web scraping module loaded but not properly initialized")
        SCRAPING_AVAILABLE = False
    ...
    # Créer des mocks complets pour éviter les erreurs
    ...
    CorpusManager = MockCorpusManager
...
@app.get("/")
async def root():
    """Root endpoint avec informations des services"""
    base_info = {
        ...
        "endpoints": {
            "ai_generation": ["/generate", "/generate-batch", "/topics"],
            "web_scraping": ["/scraping/status", "/scraping/start", "/scraping/problems", "/scraping/stats/detailed"],
            "health": ["/health"]
        }
    }
...
async def get_scraping_status():
    ...
    try:
        ...
        return {"error": f"Error getting scraping status: {str(e)}", "available": False}
...
async def get_scraped_problems(
    ...
    try:
        ...
        return {
            ...
            "error": f"Error getting problems: {str(e)}"
        }
...
async def get_detailed_scraping_stats():
    ...
    try:
        ...
        return {"error": f"Error getting detailed stats: {str(e)}"}
...
async def mark_problems_processed(request: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error marking problems as processed: {str(e)}")
...
async def search_scraped_problems(q: str, language: str = None, source: str = None):
    ...
    try:
        ...
        return {"query": q, "results": [], "count": 0, "error": f"Search error: {str(e)}"}

@app.post("/scraping/maintenance/cleanup")
async def cleanup_old_data(days: int = 30):
    """Nettoie les anciennes données"""
    if not SCRAPING_AVAILABLE or CorpusManager is None:
        raise HTTPException(status_code=503, detail="Web scraping module not available")
    
    try:
        corpus_manager = CorpusManager()
        deleted_count = corpus_manager.clear_old_data(days)
        return {"success": True, "deleted_count": deleted_count, "older_than_days": days}
    except Exception as e:
